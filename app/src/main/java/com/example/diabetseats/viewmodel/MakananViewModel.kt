...
import com.example.diabetseats.data.retrofit.ApiConfig
...
class MakananViewModel(private val dao: DiabetsDao) : ViewModel() {
    ...
    fun showSearchedFood(foodName: String) {
        _isLoading.value = true
        val client = ApiConfig.getApiService().getSearchedFood( BuildConfig.app_id, BuildConfig.app_key ,foodName)
        client.enqueue(object : Callback<MakananResponse> {
            override fun onResponse(call: Call<MakananResponse>, response: Response<MakananResponse>) {
                _isLoading.value = false
                if (response.isSuccessful) {
